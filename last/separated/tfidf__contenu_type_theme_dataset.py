...
custom_stopwords = french_stopwords + ['https', 'www', 'fr', 'texte', 'province', 'québec province', 'http']
...
text_columns = ['contenu_de_l_info', 'thématique', 'type_d_infox']
...
def compute_tfidf(documents):
    ...
    # Initialize TF-IDF vectorizer with n-grams and stopwords
    vectorizer = TfidfVectorizer(
        max_features=20,  # Limit to top 20 terms
        ngram_range=(1, 3),  # Capture unigrams, bigrams, trigrams
        stop_words=custom_stopwords,  # Remove French and custom stopwords
    ...
    try:
        ...
        print("\nTop 20 terms for contenu , type and theme (TF-IDF scores):")
...
# Generate word cloud
generate_wordcloud(term_scores, 'Word Cloud: contenu , type and theme', 'wordcloud_contenu_type_theme_dataset.png')

# Step 8: Save TF-IDF results to text file
def save_tfidf_to_text(sorted_terms, file):
    if sorted_terms:
        file.write("TF-IDF Results for contenu , type and theme:\n")
        for term, score in sorted_terms:
            file.write(f"{term}: {score:.4f}\n")

with open('tfidf_results__contenu_type_theme_dataset.txt', 'w', encoding='utf-8') as f:
    save_tfidf_to_text(sorted_terms, f)

print("\nAnalysis complete! Check 'tfidf_results_whole_dataset.txt' for TF-IDF scores and 'wordcloud_whole_dataset.png' for word cloud.")